...
func TestNewEcsLeaveSecurityGroupApi_Do(t *testing.T) {
	var ak = "<YOUR AK>"
	var sk = "<YOUR SK>"
	client := test.BuildProdClient()
	credential, _ := common.NewCredential(ak, sk)
	response, err := client.Apis.EcsLeaveSecurityGroupApi.Do(context.Background(), credential, &ecs.EcsLeaveSecurityGroupRequest{
