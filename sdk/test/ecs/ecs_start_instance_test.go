...
func TestNewEcsStartInstanceApi_Do(t *testing.T) {
	var ak = "<YOUR AK>"
	var sk = "<YOUR SK>"
	client := test.BuildProdClient()
	credential, _ := common.NewCredential(ak, sk)
	response, err := client.Apis.EcsStartInstanceApi.Do(context.Background(), credential, &ecs.EcsStartInstanceRequest{
